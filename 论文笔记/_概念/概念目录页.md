...
**共 163 条概念** · 生成时间: 2026-08-01 17:49

## 📥 待分类

- **[[Coq]]**（Rocq / Coq 8.20）  
  Rocq（原名 Coq，2025 年更名）是一个交互式定理证明器，基于归纳构造演算（Calculus of Inductive Constructions），支持形式化证明的开发和验证。
- **[[Intelligent Tutoring System]]**（ITS / 智能辅导系统 / AI Tutor）  
  利用人工智能技术模拟人类教师的一对一辅导过程，为学生提供个性化学习指导、即时反馈和适应性教学的计算机系统。
...
- **[[Polya's Problem-Solving Framework]]**（Polya框架 / Polya四步法）  
  数学家 George Polya 提出的经典数学解题方法论，包含四个步骤：理解问题（Understand）、制定计划（Plan）、执行计划（Execute）、回顾反思（Look Back）。
- **[[Scaffolding Theory]]**（支架式教学 / Scaffolding）  
  教育心理学概念，指教师在学习者现有能力水平之上提供临时性支持，帮助其完成超出当前独立能力范围的任务，随着学生能力提升逐步撤除支持。
- **[[Socratic Method]]**（苏格拉底式提问 / Socratic Questioning）  
  通过系统性提问引导学生自主发现知识和纠正错误理解的对话式教学方法，而非直接告知答案。
...
- **[[DeepScaler]]**（DeepScaleR / DeepScaler dataset）  
...
- **[[Length-Dependent Accuracy Reward]]**（长度依赖准确率奖励 / 长度正则化奖励 / length-regularized reward）  
...
- **[[Persona-Aware Thinking Reward]]**（GRPO变体 / 人格感知思考奖励）  
  在 Thinking Reward 的基础上，使 LLM Judge 根据学习者的特定特征（如残障类型、学习风格）来评估和奖励教学推理质量，实现个性化教学对齐。
...
- **[[Light-R1]]**（Light-R1-14B-DS / LightR1）  
...
- **[[DeepSeek-R1]]**（DeepSeek-R1-0528-Qwen3-8B）  
...
- **[[LLM-as-Judge]]**（LLM-as-a-Judge / LLM Judge / 大模型作为评判器）  
...
- **[[Reward Hacking]]**（奖励黑客 / Reward Gaming / Reward Over-Optimization）  
...
- **[[GPQA-Diamond]]**（GPQA Diamond / GPQA / Graduate-Level Google-Proof Q&A）  
...
- **[[LLM Judge]]**（LLM-as-Judge / LLM评估器）  
  使用 LLM（如 GPT-4o-mini）自动评估文本质量或行为规范的机制，作为 RL 训练中的非可微奖励信号源。
- **[[Likert Scale]]**（Likert评分 / Likert量表 / Likert Score）  
...
- **[[Rubric-based Evaluation]]**（Rubric Evaluation / Checklist-based Evaluation / 评分量表评估）  
  使用结构化的、多项目的评分量表（Rubric）对模型输出进行细粒度评估的方法。每条评分标准检查响应的特定维度，最终汇总为整体评分。
- **[[WBEB]]**（Well-Balanced Educational Benchmark）  
  Well-Balanced Educational Benchmark，教育领域综合评估基准，涵盖四个子维度：Subject Knowledge（学科知识）、Pedagogy Knowledge（教学知识）、Essay Scoring（作文评分）、Teacher Decision Making（教学决策）。
- **[[WizardMath]]**  
...
*自动生成时间: 2026-08-01 17:49 | 共 163 条概念笔记*